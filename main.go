package main

import (
	"log"

	sixteen "github.com/miles0o0/adventOfCode/2024/16"
)

func main() {

	// day 1
	// log.Println(one.OneMain())
	// log.Println(one.OneSec())

	// day 2
	// log.Println(two.TwoMain())
	// log.Println(two.TwoSec())

	// day 3
	// log.Println(three.PartOne())
	// log.Println(three.PartTwo())

	// day 4
	// log.Println(four.PartOne())
	// log.Println(four.PartTwo())

	// day 5
	// log.Println(five.PartOne())
	// log.Println(five.PartTwo())

	// day 6
	// log.Println(six.PartOne())
	// log.Println(six.PartTwo())

	// day 7
	// log.Println(seven.PartOne())
	// log.Println(seven.PartTwo())

	// day 8
	// log.Println(eight.PartOne())
	// log.Println(eight.PartTwo())

	// day 9
	// log.Println(nine.PartOne())
	// log.Println(nine.PartTwo())

	// day 10
	// 	log.Println(ten.PartOne())
	// 	log.Println(ten.PartTwo())

	// day 11
	// log.Println(eleven.PartOne())

	// day 12 TODO
	// log.Println(twelve.PartOne())
	// log.Println(twelve.PartTwo())

	// day 13 TODO
	// log.Println(thirteen.PartOne())
	// log.Println(thirteen.PartTwo())

	// day 14
	// log.Println(fourteen.PartOne())
	// log.Println(fourteen.PartTwo())

	// day 15 TODO
	// log.Println(fifteen.PartOne())
	// log.Println(fifteen.PartTwo())

	// day 16
	// log.Println(sixteen.PartOne())
	log.Println(sixteen.PartTwo())
}
